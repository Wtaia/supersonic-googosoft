...
export function savePlugin(params: Partial<PluginType>) {
  return request<Result<any>>('/webapp/api/chat/plugin', {
...
export function getPluginList(filters?: any) {
  return request<Result<any[]>>('/webapp/api/chat/plugin/query', {
    method: 'POST',
    data: filters,
  });
}
...
export function deletePlugin(id: number) {
  return request<Result<any>>(`/webapp/api/chat/plugin/${id}`, {
    method: 'DELETE',
  });
}
...
export function getDataSetSchema(dataSetId: number) {
  return request<Result<{ list: DimensionType[] }>>(
    `/webapp/api/chat/conf/getDataSetSchema/${dataSetId}`,
    {
      method: 'GET',
    },
  );
